...
function print_usage() {
    ...
    echo "  -l, --add-langvar   Add env. variable \$GTLANG_gle to login script."
...
if test x$langvar = xlangvar ; then
	if test x$LOGINFILE = x ; then
		...
		if test x$GTLANG_gle = x$LANGDIR ; then
			echo "\$GTLANG_gle already defined."
		else
			# Already defined with a different value:
			if test x$GTLANG_gle != x ; then
				renew=renew
				OLDLANGDIR=$GTLANG_gle
			fi

			# Add the variable to the login script:
			cp $LOGINFILE $LOGINFILE.gtbackup.${DATE}-${TIME}
			echo "export GTLANG_gle=$LANGDIR" >> $LOGINFILE
			source $LOGINFILE

			# Feedback depending on whether it was added or redefined:
			if test x$renew = xrenew ; then
			 echo "The env. variable \$GTLANG_gle has been redefined in"
			 echo "$LOGINFILE. The old value $OLDLANGDIR is no longer in use."
			else
			 echo "The env. variable \$GTLANG_gle has been added to $LOGINFILE."
...
# the current dir:
if   test x$GTLANG_gle = x ; then
	echo "WARNING: The variable \$GTLANG_gle has not been defined. You will not"
	...
	echo "$0 -l"
	echo
elif test x$GTLANG_gle != x$LANGDIR ; then
	echo "WARNING: The variable \$GTLANG_gle has the value:"
	echo "  $GTLANG_gle"
